...
for node in nodes:
    nId, cls, pairs = node
    one_node = Node(cls, nid=nId, modelId=model_name)
    for k, v in pairs:
        one_node[k] = v
...
for (id1, cls1, id2, cls2, relType) in edges:
    graph.run(query_rel.format(cls1=cls1, cls2=cls2, id1=id1, id2=id2, relType=relType, modelId=model_name))
